...
do
    ...
    --
function library:Dropdown(name, options, callback)
    local dropdown = self:CreateElement("Dropdown", name)
    dropdown.options = options or {}
    dropdown.selected = dropdown.options[1] or "No option selected"
    dropdown.callback = callback or function() end
    dropdown.open = false
    dropdown.frame.Position = UDim2.new(0,0,0,dropdown.axis)
    dropdown:Refresh()
    ...
    dropdown.frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            if dropdown.open then
                ...
                for _, element in pairs(self.openedElements) do
    ...
    dropdown.optionsFrame.InputBegan:Connect(function(input)
        input.UserInputType = Enum.UserInputType.MouseButton1
    end)

    dropdown.frame.MouseEnter:Connect(function()
        if not dropdown.open then
            dropdown.frame.BackgroundTransparency = 0.9
        end
    end)
    ...
    function dropdown:Refresh()
        dropdown.title.Text = dropdown.name .. ": " .. dropdown.selected
        dropdown.title.Size = UDim2.new(1, -5, 1, 0)
        dropdown.title.Position = UDim2.new(0, 5, 0, 0)
        ...
        for i, option in pairs(dropdown.options) do
            if not dropdown.optionButtons[i] then
                dropdown.optionButtons[i] = utility:Create("TextButton", {Vector2.new(1,1), dropdown.optionsFrame}, {
                    Text = option,
                ...
                dropdown.optionButtons[i].Text = option
                dropdown.optionButtons[i]:Remove()
            end

            dropdown.optionButtons[i].InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        dropdown.frame.Parent.Size = UDim2.new(dropdown.frame.Parent.Size.X.Scale, dropdown.frame.Parent.Size.X.Offset, 0, dropdown.axis + (dropdown.open and (#dropdown.options * 20) or 0))
    end

    dropdown.frame.Parent.Size = UDim2.new(dropdown.frame.Parent.Size.X.Scale, dropdown.frame.Parent.Size.X.Offset, 0, dropdown.axis)
